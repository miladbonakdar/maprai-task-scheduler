...
{
    public class CheckOutOfTimeCommand : ICheckOutOfTimeCommand
    {
        ...
        public CheckOutOfTimeCommand(IProjectRepository projectRepository, IUserRepository userRepository,
        ...
        {
            throw new AppDefaultException(new NotImplementedException(), "CheckOutOfTimeCommand CreateActions");
        }
        ...
        public async Task NotifyProblems()
        {
            var notify = await _projectRepository.GetInvalidProjectsNotifies(_invalidProjs.Select(item => item.ProjectID).ToList(),
                NotifySetting.NotifyTypeUniqueName.OutOfTimeProject);
        ...
        {
            var projs = await _projectRepository.GetValidActiveProjects(NotifySetting.NotifyTypeUniqueName.OutOfTimeProject);
            if (projs.Count == 0) return projs;
            List<Project> invalids = new List<Project>();
            foreach (var project in projs)
            {
                if (project.RemainingTime != null && project.RemainingTime.Value < (DateTime.Now - project.CreationDate).TotalMinutes)
                {
